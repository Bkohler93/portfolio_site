const projects = [
  {
    ...
    longDescription: [
      "This app uses object detection to create an initial authorization screen for users. Users can only login when they take a picture of an identifying object or symbol, proving that they are in that location. This opens up the possibility of creating an app where users are and feel an extra connection between other users. When people are doing things they often have inspiring ideas or just a funny thought, and this app can be where people can share those ideas to people who are in a similar experience, at a similar location, as themselves. To accomplish this, I used the open source and cross-platform machine learning framework ML.NET to run Tiny Yolov2, an object detection model.",
